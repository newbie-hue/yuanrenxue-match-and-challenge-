...
class AB():
    ...
    def __init__(self):
        self.url = r"https://match.yuanrenxue.com/api/match/4?page="
        self.headers = {
    ...
    def get_response(self,i):
        response = requests.get(url=self.url+str(i), headers=self.headers).json()
        info = response["info"]
        key = response["key"]
        value = response["value"]
        price_list = self.handle_response_get_price(info,key,value)
        return price_list



    def handle_response_get_price(self,info, key, value):
        key_value = key + value
        j_key = base64.b64encode(key_value.encode("utf-8"))
        #print(str(j_key,"utf-8").replace("=",""))
        jk_ey = str(j_key,"utf-8").replace("=","")
        jke_y = hashlib.md5(jk_ey.encode())
        jkey = jke_y.hexdigest()
        price_list = []
        etree_ = etree.HTML(info)
        td_list = etree_.xpath("//td")
        for td in td_list:
            num_dict_list = []
            img_list = td.xpath("./img")
            for img in img_list:
                num = self.img_dic[img.xpath("./@src")[0]]
                clas = img.xpath("./@class")[0]
                if jkey in clas:
                    continue
                else:
                    item = {}
                    ...
                    item[num] = style
                    num_dict_list.append(item)
            num_list = ["","","",""]
            for index,val in enumerate(num_dict_list):
                #print(index,float(list(val.values())[0])/11.5)
                num_location = int(index + float(list(val.values())[0])/11.5)
                num_value = int(list(val.keys())[0])
                num_list[num_location] = str(num_value)

            price = int("".join(num_list))
    ...
    def get_sum(self):
        price_list = []
        for i in range(1,6):
            price_list_ = self.get_response(i)
            price_list += price_list_
